...
import {closeSingleConnectionAPI, fetchProxiesAPI, fetchProxyProvidersAPI, proxyGroupLatencyTestAPI, proxyLatencyTestAPI, proxyProviderHealthCheckAPI, selectProxyInGroupAPI, updateProxyProviderAPI,} from '~/apis'
import {useStringBooleanMap} from '~/helpers'
import {autoCloseConns, latencyQualityMap, latencyTestTimeoutDuration, latestConnectionMsg, restructRawMsgToConnection, urlForIPv6SupportTest, urlForLatencyTest,} from '~/signals'
...
const setProxiesInfo = (
  ...
  proxies.forEach((proxy) => {
    ...
    const latency = lastDelay(proxy, urlForLatencyTest()) || latencyQualityMap().NOT_CONNECTED
    newLatencyMap[proxy.name] = latency

    const proxyIPv6Support = (lastDelay(proxy, urlForIPv6SupportTest(), false) ?? 0) > 0
