...
import {BackendVersion, Config, Proxy, ProxyProvider, Rule, RuleProvider,} from '~/types'
...
export const [updatingGEODatabases, setUpdatingGEODatabases] = createSignal(false)
...
export const isUpdateAvailableAPI = async (versionResponse: string) => {
  ...
  if (channel === 'meta') {
    const {assets} = await ky
      .get('https://api.github.com/repos/devsolux/clash-meta/releases/latest')
      .json<{
        assets: {
          name: string
        }[]
  ...
  if (channel === 'alpha') {
    const {assets} = await ky
      .get(
        'https://api.github.com/repos/devsolux/clash-meta/releases/tags/Prerelease-Alpha',
      )
      .json<{
        assets: {
          name: string
